...
my $custom_chars = {
	...
	17 => [ # Triforce, this one made me need larger widths
		0b0000010000000000,
		0b0000111000000000,
		0b0001111100000000,
		0b0010000010000000,
...

{
	...
	sub bitmap {
		...
		return [map {$_ << 16} $custom_chars->{$_[0]->$*}->@*];
	}
...
sub transpose_glyph {
	...
	my @trans_bits = grep {$_ !~ /0b0+$/} map {"0b".join '', $_->@*} @new_bits;
...
const PROGMEM uint8_t font_glyphs[] = {
    %B%
};
const PROGMEM uint8_t font_starts[] = {%S%};
const PROGMEM uint8_t font_widths[] = {%W%};
